...
import 'package:bismillah_constructions/shared/data/repositories/ledger_repository.dart'
    show MaterialTrialGroupBy, MaterialTrialRow;
...
class PdfGenerator {
  /// Delivers a finished PDF to the user. On mobile this opens the native
  /// print/preview sheet (from which they can save or print). On desktop
  /// (Windows/Linux/macOS) there is no such sheet, so a "Save As" dialog is
  /// shown and the bytes are written to the chosen location; if the user
  /// cancels, nothing happens.
  static Future<void> _emit(Uint8List bytes, String name) async {
    if (!kIsWeb &&
    ...
    await Printing.layoutPdf(onLayout: (PdfPageFormat f) async => bytes, name: name);
  }

  static Future<void> previewIncomeStatement(IncomeStatementData d) async {
    await _emit(await _buildIncomeStatement(d), 'Income Statement');
  }

  static Future<void> previewBalanceSheet(BalanceSheetData d) async {
    await _emit(await _buildBalanceSheet(d), 'Balance Sheet');
  }

  static Future<void> previewSupplierLedger(SupplierLedgerData d) async {
    await _emit(await _buildSupplierLedger(d),
        'Material Supplier Ledger — ${d.supplierName}');
  }

  static Future<void> previewBankLedger(BankLedgerData d) async {
    await _emit(await _buildBankLedger(d), 'Bank Ledger — ${d.bankName}');
  }

  static Future<void> previewProjectLedger(ProjectLedgerData d) async {
    await _emit(
        await _buildProjectLedger(d), 'Project Ledger — ${d.projectName}');
  }
  ...
  static Future<void> previewWorkerLedger(WorkerLedgerData d) async {
    await _emit(await _buildWorkerLedger(d), 'Wage Ledger — ${d.workerName}');
  }
  ...
  // ---------------- Material Trial Balance ----------------

  static String qtyText(MaterialTrialRow r) {
  ...
  static Future<void> previewTrialBalance({
    ...
    required MaterialTrialGroupBy groupBy,
    required List<MaterialTrialRow> rows,
    required DateTime generatedAt,
  }) async {
    await _emit(
      await _buildTrialBalance(scope, groupBy, rows, generatedAt),
      'Trial Balance — $scope',
    );
  }

  static Future<Uint8List> _buildTrialBalance(
    String scope,
    MaterialTrialGroupBy groupBy,
    List<MaterialTrialRow> rows,
    DateTime generatedAt,
  ) async {
    final doc = pw.Document();
    final byMaterial = groupBy == MaterialTrialGroupBy.material;
    final keyHeader = byMaterial ? 'Material' : 'Supplier';
    final total = rows.fold<double>(0, (a, r) => a + r.amount);

    final tableRows = <List<String>>[
      ['Project', keyHeader, 'Quantity', 'Amount'],
      for (final r in rows)
        [r.projectName, r.label, qtyText(r), fmtMoney(r.amount)],
    ];

    doc.addPage(pw.MultiPage(
      build: (ctx) => [
        _header('Trial Balance — Material Spend',
            'Scope: $scope  ·  Grouped by ${byMaterial ? 'material' : 'supplier'}  ·  Generated ${fmtDateTime(generatedAt)}'),
        pw.SizedBox(height: 12),
        pw.TableHelper.fromTextArray(
          headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold),
        ...
        pw.Align(
          alignment: pw.Alignment.centerRight,
          child: pw.Text('Total Amount: ${fmtMoney(total)}',
