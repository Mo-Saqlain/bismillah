...
/// How the material Trial Balance groups its rows within each project.
enum MaterialTrialGroupBy { material, supplier }

/// One aggregated line in the material Trial Balance report: for a project,
/// the total quantity and amount spent on a single material type (or with a
/// single supplier), sourced from `material_inventory`.
class MaterialTrialRow {
  /// Project this line rolls up under.
  final String projectId;
  final String projectName;

  /// Material type name (material grouping) or supplier name (supplier
  /// grouping). Counter purchases with no supplier get a fixed label.
  final String label;

  /// Material unit for the material grouping (may be mixed → 'mixed'); empty
  /// for the supplier grouping.
  final String unit;

  /// Summed quantity (rows logged without a quantity contribute 0).
  final double quantity;

  /// Summed amount (`total_cost`).
  final double amount;

  const MaterialTrialRow({
    required this.projectId,
    required this.projectName,
    required this.label,
    required this.unit,
